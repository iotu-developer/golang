...
// add by yuanxiang 2017-06-02
// 返回集合中的count个随机元素
func (cache *Cache) SRandMEMBER(key string, count int) (res []string, err error) {
	conn := cache.RedisPool().Get()
	defer conn.Close()
	res, err = redis.Strings(conn.Do("SRANDMEMBER", key, count))
	if nil != err && strings.Contains(err.Error(), "nil returned") {
		err = nil
		return
	}
...
// add by yuanxiang 2017-01-07
// batch add slice
func (cache *Cache) SBatchAddSlice(key string, items []string) (err error) {
	conn := cache.RedisPool().Get()
	defer conn.Close()
	args := make([]interface{}, len(items)+1)
	args[0] = key
...
// update by baixiao 2017-3-17
func (cache *Cache) ZCount(key string, min, max int) (int64, error) {
	conn := cache.RedisPool().Get()
	defer conn.Close()
	res, err := redis.Int64(conn.Do("ZCOUNT", key, min, max))
	if nil != err && !strings.Contains(err.Error(), "nil returned") {
		err = errcode.NewInternalError(errcode.CacheErrCode, err)
	}
	return res, err
...
//update by zhaohang 2018-01-04
func (cache *Cache) SetNx(key string, bytes interface{}) (int, error) {
	var err error
	var n int
	conn := cache.RedisPool().Get()
	defer conn.Close()
	n, err = redis.Int(conn.Do("SETNX", key, bytes))
	if nil != err && !strings.Contains(err.Error(), "nil returned") {
		err = errcode.NewInternalError(errcode.CacheErrCode, err)
	}
	return n, err
...
// update by gaoxianru 2018-08-07
// 同时使用EX和NX参数，实现分布式锁
func (cache *Cache) SetNxEx(key string, bytes interface{}, timeout int) (exist bool, err error) {
	conn := cache.RedisPool().Get()
	defer conn.Close()

	_, err = redis.String(conn.Do("SET", key, bytes, "EX", timeout, "NX"))
	if nil != err {
...
func (cache *Cache) SetNxPX(key string, bytes interface{}, timeout int) (exist bool, err error) {
	conn := cache.RedisPool().Get()
	defer conn.Close()

	_, err = redis.String(conn.Do("SET", key, bytes, "PX", timeout, "NX"))
	if nil != err {
		if !strings.Contains(err.Error(), "nil returned") {
			err = errcode.NewInternalError(errcode.CacheErrCode, err)
		} else {
