...
func FormatUserAgent(user_agent string) map[string]interface{} {
	...
	// iOS12初始化WebView时可能拿不到ua信息，跟前端约定多带一个sa-sdk-ios表示来自APP内部
	dealed_user_agent = strings.Replace(dealed_user_agent, ";sa-sdk-ios", "", -1)
...
// ios是否是app store 版本
// whiteList true 需要判断白名单
func IsIosFromAppStore(userId string, userAgent map[string]interface{}, header http.Header, whiteList bool) (bool, error) {
	platform, ok := userAgent["platform"]
	if !ok {
		return false, fmt.Errorf("user id %s  get platform failed no platform field", userId)
	}
	p, ok := platform.(int)
	if !ok {
		return false, fmt.Errorf("user id %s  get platform failed invalid platform type", userId)
	}
	if p != 0 {
		// 不是ios的 认为都是app store 版本
		return true, nil
	}
	version, ok := userAgent["version"]
	if !ok {
		return false, fmt.Errorf("user id %s  get version failed no version field", userId)
	}
	v, ok := version.(string)
	if !ok {
		return false, fmt.Errorf("user id %s get version failed invalid version type", userId)
	}
	old, err := CompareVersion(v, "9.7.0", 3)
	if err != nil {
		return false, fmt.Errorf("user id %s get version failed invalid version format", userId)
	}
	if old {
		// 低于9.7.0的 只能认为都是 app store版本
		return true, nil
	}
	if header.Get("from_channel") == "appstore" {
		return true, nil
	}
	if whiteList && utils.UserGrayTest(userId, "iap") {
		clog.Logger.Info("UserGrayTest success user id %s", userId)
		return true, nil
	}
	return false, nil
}
