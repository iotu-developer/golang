...
func (r *htmlDebugRender) Render(w http.ResponseWriter, code int, data ...interface{}) error {
	...
	t := template.New("")
	if len(r.files) > 0 {
		t = template.Must(t.Funcs(r.FuncMap).ParseFiles(r.files...))
	}

	for _, glob := range r.globs {
		t = template.Must(t.Funcs(r.FuncMap).ParseGlob(glob))
	}
...
func HTMLDebugWithFuncMap(funcMap template.FuncMap) *htmlDebugRender {
	HTMLDebug.FuncMap = funcMap
	return HTMLDebug
}
