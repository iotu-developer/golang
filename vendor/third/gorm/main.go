...
type DB struct {
	...
	dontPrintQuerySql bool // 在log打开的情况下，控制是否打印query sql. dontPrintQuerySql初始时是false，即打印query sql
}
...
func (s *DB) PrintQuerySql(enable bool) *DB {
	s.dontPrintQuerySql = !enable
	return s
}
